...
- **GitHub 100 MB file limit & Git LFS lock timeout.** SQLite databases (like `state.db`) quickly exceed 100 MB over active usage, triggering GitHub pre-receive hook rejections (`GH001: Large files detected`). Git LFS can hang or fail with TLS timeouts on locking API in restricted regions. **Solution:** Automatically compress and chunk large databases (e.g. `gzip` into 40MB chunks via `split`) before staging in git, or run database vacuuming during weekly cleanup.
...
- **Terminal yielding to background on user message during push**: Running `backup_hermes.sh` takes 10–20 seconds over the network. If the user sends a message while it is running, Hermes terminal automatically moves the process to background (`yielded_to_background`). Never re-run the command or freeze; immediately call `process_manage(action='wait', session_id=...)` to await exit, then verify `git log -1` before responding.
